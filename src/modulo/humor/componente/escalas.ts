 ...
 * (RegistroDoDia) e a edição de um dia antigo (FormularioDeEdicao) mostram exatamente
 * os mesmos botões — e um emoji divergente entre as duas telas confundiria.
 */

export type OpcaoDeEscala = {
  value: number
  emoji: string
  label: string
}

export const HUMORES: OpcaoDeEscala[] = [
...
export const ENERGIAS: OpcaoDeEscala[] = [
...
export function rotuloDaEscala(options: OpcaoDeEscala[], value: number) {
  return options.find(option => option.value === value)?.label || '—'
}

/** Emoji de um valor 1-5 */
export function emojiDaEscala(options: OpcaoDeEscala[], value: number) {
